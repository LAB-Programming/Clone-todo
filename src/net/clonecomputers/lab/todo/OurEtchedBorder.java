 ...
 * @author Gavin Yancey
 */
public class OurEtchedBorder extends AbstractBorder
{
    /** Raised etched type. */
     ...
     */
    public OurEtchedBorder(int etchType) {
    	this(etchType, TOP | BOTTOM | LEFT | RIGHT);
    }

    public OurEtchedBorder(int etchType, int side){
        this(etchType, side, null, null);
    }
     ...
     */
    public OurEtchedBorder(Color highlight, Color shadow) {
        this(LOWERED, highlight, shadow);
    }
    
    public OurEtchedBorder(int etchType, Color highlight, Color shadow){
    	this(etchType, TOP | BOTTOM | LEFT | RIGHT);
    }
    ...
    public OurEtchedBorder(int etchType, int side, Color highlight, Color shadow) {
    	this.side = side;
    ...
    public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
    	System.out.println("Painting Our Etched Border");
        int w = width;
        int h = height;
        g.translate(x, y);
        g.setColor(etchType == LOWERED? getShadowColor(c) : getHighlightColor(c));
        //g.drawRect(0, 0, w-2, h-2);
        if((side & TOP) != 0) g.drawLine(0,0,w-2,0);
        if((side & LEFT) != 0) g.drawLine(0,0,0,h-2);
        if((side & RIGHT) != 0) g.drawLine(w-2,0,w-2,h-2);
        if((side & BOTTOM) != 0) g.drawLine(0,h-2,w-2,h-2);
        g.setColor(etchType == LOWERED? getHighlightColor(c) : getShadowColor(c));
        if(false || (side & LEFT) != 0) g.drawLine(1, h-3, 1, 1);
        if(false || (side & TOP) != 0) g.drawLine(1, 1, w-3, 1);
        if(false || (side & BOTTOM) != 0) g.drawLine(0, h-1, w-1, h-1);
        if(false || (side & RIGHT) != 0) g.drawLine(w-1, h-1, w-1, 0);
        g.translate(-x, -y);
    }
    /**
